defmodule Users do
  ...
  def insert_user(table, user) do
    table
    |> table_exists?()
    |> handle_insert(table, user)
  end
  ...
  defp handle_delete({:error, %{message: "User not found in erlang team storage"}}, _table, _user), do: {:user_not_found}
  ...
  defp handle_search_user_by_key([]), do: {:error, %{message: "User not found in erlang team storage"}}
